...
const ALL_COLUMNS = [
  ...
  { key: "discount", label: "Discount (%)", group: "Pricing", always: false },
  ...
  {
    key: "partnerEnabled",
    label: "Partner Stock Enabled",
    group: "Stock",
    always: false,
  },
  {
    key: "partnerQty",
    label: "Partner Stock Qty",
    group: "Stock",
    always: false,
  },
...
export default function ProductExportModal({
  ...
  const applyPreset = (presetId) => {
    ...
    set({
      preset: presetId,
      selectedColumns: preset.keys ? preset.keys : config.selectedColumns, // keep existing for 'custom'
    });
  };
  ...
  const toggleGroup = (group) => {
    const groupKeys = ALL_COLUMNS.filter(
      (c) => c.group === group && !c.always,
    ).map((c) => c.key);
    const allSelected = groupKeys.every((k) =>
      config.selectedColumns.includes(k),
    );
    set({
      preset: "custom",
  ...
  const scopeLabel = () => {
    switch (config.scope) {
      case "page":
        return `Current page (${currentPageCount} products)`;
      case "filtered":
        return `All filtered results (${totalProducts} products)`;
      case "all":
        return `All products in catalog`;
      case "custom":
        return `Custom: ${config.customLimit} products from page ${config.customPage}`;
      default:
        return "";
    }
  };
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md p-8 text-center">
          ...
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
            {t("productExport.exportComplete")}
          </h3>
          <p className="text-gray-500 dark:text-gray-400 mb-6">
            {config.exportedCount} products exported as{" "}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                {t("productExport.exportProducts")}
              </h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Configure and download your product data
              </p>
            </div>
          </div>
          <button
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                },
                {
                  ...
                  title: "PDF",
                  sub: "Printable report with colour status",
                },
              ].map((f) => (
                <button
                  key={f.id}
          ...
          <section>
              ...
              {[
                {
                  ...
                  badge: `${currentPageCount} products`,
                  badgeColor: "blue",
                },
                {
                  id: "filtered",
                  label: `All filtered / search results`,
                  badge: `${totalProducts} products`,
                  badgeColor: "green",
                },
                {
                  id: "all",
                  label: `Entire product catalog`,
                  badge: null,
                  badgeColor: "purple",
                },
                {
                ...
                >
                  ...
                  {s.badge && (
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-semibold bg-${s.badgeColor}-100 text-${s.badgeColor}-700 dark:bg-${s.badgeColor}-900/30 dark:text-${s.badgeColor}-300`}
                    >
                      {s.badge}
                    </span>
                  )}
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      set({ customLimit: parseInt(e.target.value) })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      set({
                        customPage: Math.max(1, parseInt(e.target.value) || 1),
                      })
                    }
                    className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-green-500"
                  />
            ...
            {activeFilterCount > 0 && (
              <div className="mt-3 flex items-center gap-2 p-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg text-xs text-amber-700 dark:text-amber-300">
                ...
                {activeFilterCount > 1 ? "s" : ""} applied — "Filtered results"
          ...
          <section>
            ...
            <div className="space-y-2 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
              {GROUPS.map((group) => {
                ...
                const selectedInGroup = groupCols.filter((c) =>
            ...
            <p className="mt-2 text-xs text-gray-400">
              {config.selectedColumns.length} of {ALL_COLUMNS.length} columns
              selected
            </p>
          </section>
        </div>
        ...
        <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 p-3 mb-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg text-sm text-gray-600 dark:text-gray-400">
            ...
            <span>
              ...
              with{" "}
              <strong className="text-gray-800 dark:text-gray-200">
                {config.selectedColumns.length} columns
              </strong>
            </span>
          </div>
