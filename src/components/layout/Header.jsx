...
import {
  Moon, Sun, Bell, ChevronDown, User, Settings, LogOut, RefreshCw,
...
const NOTIFICATION_TYPE_COLORS = {
  ...
  USER_REGISTRATION: 'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300',
  PASSWORD_RESET: 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300',
  PRODUCT: 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300',
  STOCK: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300',
...
const Header = ({ currentUser, darkMode, onToggleDarkMode }) => {
  ...
  const getUserAvatar = (user) => {
    if (!user) return null;
    if (user.avatar) {
      return <img src={user.avatar} alt={user.name} className="h-8 w-8 rounded-full object-cover" />;
    }
    return (
  ...
  const handleProfileSettings = () => { setShowProfileDropdown(false); navigate('/dashboard/settings'); };
    ...
    <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 sticky top-0 z-40">
      <div className="flex items-center justify-end gap-6">
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button
              onClick={() => { setShowNotifications(!showNotifications); if (!showNotifications) fetchNotifications(1); }}
              ...
              <div className="absolute right-0 mt-2 w-96 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl z-50 overflow-hidden">
                ...
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => { navigate('/admin/dashboard/notifications'); setShowNotifications(false); }}
                      className="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400"
                      title="View all"
                    >
                ...
                <div className="max-h-80 overflow-y-auto">
                  {loading && notifications.length === 0 ? (
                    ...
                    <div className="p-6 text-center text-gray-500 dark:text-gray-400 text-sm">No notifications</div>
                  ) : (
                    notifications.slice(0, 10).map((n) => (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          {/* type badge dot */}
                          <div className={`mt-1 w-2 h-2 rounded-full flex-shrink-0 ${PRIORITY_DOT[n.priority] || PRIORITY_DOT.medium}`} />
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${NOTIFICATION_TYPE_COLORS[n.type] || NOTIFICATION_TYPE_COLORS.default}`}>
                                {n.type?.replace('_', ' ')}
                              </span>
          ...
          <div className="relative">
            ...
            {showProfileDropdown && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-50">
                ...
                <div className="py-1">
                  <button onClick={handleProfileSettings} className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
                  ...
                  <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2">
      ...
      {(showProfileDropdown || showNotifications) && (
        <div
          ...
          onClick={() => { setShowProfileDropdown(false); setShowNotifications(false); }}
        />
      )}
    </header>
  );
