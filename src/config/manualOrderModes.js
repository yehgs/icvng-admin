/**
 * admin/src/config/manualOrderModes.js
 *
 * Mirror of icvng-server/config/manualOrderModes.js. Keep the two in sync —
 * if you change a mode rule, change both.
 *
 * The admin uses this to LOCK the order-type/order-mode selectors for a
 * SALES agent, so they see immediately what they can create rather than
 * picking something the server will reject. The lock is UX; the server
 * derives the mode from the account regardless of what the client sends.
 *
